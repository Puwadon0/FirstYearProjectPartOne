...
def expense_list():
    ...
    act_map = {a["act_id"]: {"name": a["act_name"], "budget": a["act_cost"] or 0} for a in acts}
    ...
    with get_activity_db() as conn:
        if filename:
            ...
            ''', (
                ...
                request.form.get('location'), request.form.get('address'),
                float(request.form.get('cost') or 0),
                request.form.get('objective'), request.form.get('steps'),
                request.form.get('notes'), request.form.get('responsiblePerson'),
